...
def GetLayer( dict):
    ...
    elif dict["layerType"] == "Dropout":
        layer = KL.Dropout(**dict["Parameters"],name=dict["layerName"])
    elif dict["layerType"] == "ZeroPadding2D":
        layer = KL.ZeroPadding2D(**dict["Parameters"],name=dict["layerName"])

    #Weird Math Layers
    elif dict["layerType"] == "LogSoftMax":
        layer = tf.nn.log_softmax
    elif dict["layerType"] == "Clip":
        layer = KL.Lambda(lambda x: tf.clip_by_value(x,**dict["Parameters"]))
    elif dict["layerType"] == "Log":
        layer = tf.math.log
    ...
    elif dict["layerType"] == "ChannelFilter":
        layer= ChannelFilter( **dict["Parameters"],name=dict["layerName"])
